...
$(document).ready(function() {
  loadScript("https://maps.googleapis.com/maps/api/js?key=" + "AIzaSyCA-XzNjxHCoz2bGuvILz5fpoh3qQ6Kgr8" + "&libraries=drawing&callback=initMap");
});
...
function initMap() {
  ...
  setInterval(addInventory, 500);
  setInterval(updateTrainer, 500);
  setInterval(addCatchable, 500);
}
  ...
  if (menu == 1) {
    ...
    var current_user_data = user_data[users[user_id]].stats[0];
      out += '<div class="row"><div class="col s12"><h5>' +
              ...
              current_user_data.inventory_item_data.player_stats.next_level_xp) * 100 +
              '%"></div></div>' +
               (current_user_data.inventory_item_data.player_stats.experience/
              current_user_data.inventory_item_data.player_stats.next_level_xp) * 100     +
              '%<br><br>Exp: ' +
